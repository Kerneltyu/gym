...
def get_status(_observation):
 ...
 #Q値の更新
def update_q_table(_env, _q_table, _e_table, _action, _observation, _next_observation, _next_action, _reward, _episode):
     alpha = 0.2
     gamma = 0.99
     delta = 0
     lam = 0.9
     ...
     next_q_value = _q_table[next_position][next_velocity][_next_action]

     delta = _reward + gamma * next_q_value - q_value
     _e_table[position][velocity][_action] + 1

     for i in range(50):
         for j in range(50):
             for k in range(3):
                 _q_table[i][j][k] = _q_table[i][j][k] + alpha * delta * _e_table[i][j][k]
                 _e_table[i][j][k] = gamma * lam * _e_table[i][j][k]

     _q_table[position][velocity][_action] = q_value + alpha * (_reward + gamma * next_q_value - q_value)

     return [_q_table, _e_table]
...
if __name__ == '__main__':
    ...
    e_table = np.zeros((50, 50, 3)) # 適格度トレース
    for i_episode in range(5001):
        ...
        for t in range(200):
            ...
            next_action = get_action(env, q_table, next_observation, i_episode)
            #価値更新
            q_table, e_table = update_q_table(env, q_table, e_table, action, observation, next_observation, next_action, reward, i_episode)
            total_reward += reward
            action = next_action
